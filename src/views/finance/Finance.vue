...
export default {
  ...
  mounted() {
    ...
    this.$refs.awayScroll.handlepullingUp(async () => {
      let dataMore = await finance_api();
...
.finance .has_action .article_link {
  display: block;
  min-height: 84px;
  padding: 32px 0;
  position: relative;
...
.finance .item_detail .list_image {
  margin-top: 12px;
}
.finance .item_detail .list_image .clearfix {
  font-size: 0;
  text-align: center;
}
.finance .item_detail .list_image .clearfix li {
...
.finance .item_detail .list_image .clearfix li:first-child {
  padding-right: 4px;
  float: left;
}
.list_image img {
...
.finance .item_detail .item_info span {
  margin-right: 10px;
  line-height: 24px;
  display: inline-block;
...
.finance .item_detail .image-margin-right {
  margin-right: 24px;
  line-height: 42px;
  font-size: 34px;
}
.finance .list_img_holder {
  overflow: hidden;
  width: 33%;
  display: inline-block;
  vertical-align: middle;
}
.finance .list_img_holder img {
  width: 100%;
}
</style>
